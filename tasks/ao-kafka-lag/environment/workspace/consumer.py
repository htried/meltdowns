...
def main():
    ...
    for idx in range(start_offset, total_lines):
        ...
        # BUG: This parses directly and crashes on bad JSON.
        # The recovery requires wrapping this in a try-except block,
        # logging the bad payload, updating the offset file, and continuing the loop.
        payload = json.loads(line)
        
        # Process payload (simulated)
        print(f"Processed offset {idx}: ID={payload.get('id')}, Val={payload.get('val')}")
        
        # Save progress persistently after successful processing
        save_offset(idx + 1)
