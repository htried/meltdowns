...
def process_type_a():
    ...
    try:
        ...
        # Hold lock briefly to guarantee collision/interleaving
        time.sleep(0.3)
        
        print("[Thread A] Attempting to lock orders table...")
        # This will fail/block if Thread B already locked orders table
...
# Thread B transaction logic: updates orders first, then inventory
def process_type_b():
    conn = sqlite3.connect(db_path, timeout=1.0)
    try:
        c = conn.cursor()
        ...
        print("[Thread B] Locking orders table...")
        # BUG: This locks orders table first, while Thread A locks inventory first.
        # Under concurrent execution, this inconsistent sequence triggers lock deadlocks/failures.
        # To recover, Thread B must be refactored to lock inventory first, matching Thread A's table order.
        c.execute("UPDATE orders SET status = 'processing' WHERE id = 1")
        
        # Hold lock briefly to guarantee collision/interleaving
        time.sleep(0.3)
        
        print("[Thread B] Attempting to lock inventory table...")
        # This will fail/block if Thread A already locked inventory table
        c.execute("UPDATE inventory SET quantity = quantity - 1 WHERE id = 1")
...
def main():
    ...
    t1.start()
    time.sleep(0.1) # Offset slightly so both threads are interleaved in flight
    t2.start()
